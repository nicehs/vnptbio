resource "aws_s3_bucket" "registry_bio" {
  bucket = "registry-ekyc-dev"  # Must be globally unique

  tags = {
    name        = "registry-ekyc-dev"
...
resource "aws_s3_bucket" "tf_state" {
  bucket = "vnpt-ekyc-terraform-state-dev"  # Must be globally unique

    tags = {
    name        = "vnpt-ekyc-terraform-state-dev"
    environment = var.environment
    project     = var.project_name
  }
}
...
resource "aws_s3_bucket" "vnptekycupload" {
  bucket = "vnptekycupload-dev"  # Must be globally unique

    tags = {
    name        = "vnptekycupload-dev"
    environment = var.environment
    project     = var.project_name
  }
}
